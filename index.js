...
function compression(options) {
  ...
  return function compression(req, res, next) {
    ...
    onHeaders(res, function onResponseHeaders() {
      ...
      const methods = ['br', 'gzip', 'deflate', 'identity'];
      const preferredMethods = ['br', 'gzip', 'deflate'];

      if (!zlib.createBrotliCompress) {
        methods.splice(0, 1);
        preferredMethods.splice(0, 1);
        debug('brotli not supported');
      }
      let method = accept.encoding(methods);

      // ALWAYS prefer brotli
      if (method !== 'br' && accept.encoding(['br'])) {
        method = 'br';
      }
      ...
      switch (method) {
        ...
        default: debug('unknown compression method: %s', method);
      }
